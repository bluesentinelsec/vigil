...
static char *doc_render_helper(VigilDocTest *f, const char *src_text, const char *symbol) {
    ...
    if (vigil_source_registry_register_cstr(
            &f->registry, "test.vigil", src_text, &source_id, &f->error) != VIGIL_STATUS_OK) return NULL;
    if (vigil_lex_source(&f->registry, source_id, &f->tokens, &f->diagnostics, &f->error) != VIGIL_STATUS_OK) return NULL;
    if (vigil_doc_extract(NULL, "test.vigil", 9, src_text, strlen(src_text),
                          &f->tokens, &module, &f->error) != VIGIL_STATUS_OK) return NULL;
...
TEST_F(VigilDocTest, MissingSymbolReturnsError) {
    ...
    ASSERT_EQ(vigil_source_registry_register_cstr(
        &F->registry, "test.vigil", src, &source_id, &F->error), VIGIL_STATUS_OK);
    ASSERT_EQ(vigil_lex_source(&F->registry, source_id, &F->tokens, &F->diagnostics, &F->error),
              VIGIL_STATUS_OK);

    vigil_doc_module_t module;
    ASSERT_EQ(vigil_doc_extract(NULL, "test.vigil", 9, src, strlen(src),
                                &F->tokens, &module, &F->error), VIGIL_STATUS_OK);

    char *text = NULL;
    size_t length = 0;
    EXPECT_EQ(vigil_doc_render(&module, "missing", &text, &length, &F->error),
...
TEST_F(VigilDocTest, ModuleNameFromPath) {
    ...
    ASSERT_EQ(vigil_source_registry_register_cstr(
        &F->registry, "lib/utils.vigil", src, &source_id, &F->error), VIGIL_STATUS_OK);
    ASSERT_EQ(vigil_lex_source(&F->registry, source_id, &F->tokens, &F->diagnostics, &F->error),
              VIGIL_STATUS_OK);

    vigil_doc_module_t module;
    ASSERT_EQ(vigil_doc_extract(NULL, "lib/utils.vigil", 14, src, strlen(src),
                                &F->tokens, &module, &F->error), VIGIL_STATUS_OK);
