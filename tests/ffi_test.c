...
TEST(FFIDlopen, SymNotFound) {
    ...
    void *sym = NULL;
    vigil_status_t s = vigil_platform_dlsym(handle, "nonexistent_symbol_xyz",
                                           &sym, &error);
...
TEST(FFIDlopen, OpenBadPath) {
    void *handle = NULL;
    vigil_error_t error = {0};
    vigil_status_t s = vigil_platform_dlopen("/no/such/library.so",
                                            &handle, &error);
    EXPECT_TRUE(s != VIGIL_STATUS_OK);
}
...
static void ffi_vm_setup(vigil_runtime_t **rt, vigil_vm_t **vm,
                          vigil_error_t *error) {
...
static void push_str(vigil_vm_t *vm, vigil_runtime_t *rt, const char *s,
                      vigil_error_t *error) {
...
TEST(FFIModule, CallVoidReturn) {
    ...
    push_str(vm, rt, FFI_TESTLIB_PATH, &error);
    fn_open(vm, 1, &error);
    int64_t lib = vigil_nanbox_decode_int(
    ...
    fn_bind(vm, 3, &error);
    int64_t h = vigil_nanbox_decode_int(
        vigil_vm_stack_get(vm, vigil_vm_stack_depth(vm) - 1));
    vigil_vm_stack_pop_n(vm, 1);
    ...
    ASSERT_EQ((int)fn_call(vm, 7, &error), (int)VIGIL_STATUS_OK);
    int64_t result = vigil_nanbox_decode_int(
        vigil_vm_stack_get(vm, vigil_vm_stack_depth(vm) - 1));
    vigil_vm_stack_pop_n(vm, 1);
...
TEST(FFIModule, CallF_Float) {
    ...
    double result = vigil_nanbox_decode_double(
        vigil_vm_stack_get(vm, vigil_vm_stack_depth(vm) - 1));
    vigil_vm_stack_pop_n(vm, 1);
...
TEST(FFIModule, CallS_String) {
    ...
    const vigil_object_t *obj =
        (const vigil_object_t *)vigil_nanbox_decode_ptr(r_val);
...
TEST(FFIModule, SymLookup) {
    ...
    int64_t sym = vigil_nanbox_decode_int(
        vigil_vm_stack_get(vm, vigil_vm_stack_depth(vm) - 1));
    vigil_vm_stack_pop_n(vm, 1);
    EXPECT_TRUE(sym != 0);
