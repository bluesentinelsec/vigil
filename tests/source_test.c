...
TEST(VigilSourceTest, RegisterCopiesOwnedPathAndText) {
    ...
    ASSERT_EQ(
        vigil_source_registry_register(
            &registry,
            path,
            strlen(path),
            text,
...
TEST(VigilSourceTest, MultipleRegistrationsUseStableSourceIds) {
    ...
    ASSERT_EQ(
        vigil_source_registry_register_cstr(&registry, "a.vigil", "a", &first, &error),
        VIGIL_STATUS_OK
    );
    ASSERT_EQ(
        vigil_source_registry_register_cstr(&registry, "b.vigil", "b", &second, &error),
        VIGIL_STATUS_OK
    );
    ...
    EXPECT_STREQ(
        vigil_string_c_str(&vigil_source_registry_get(&registry, second)->path),
        "b.vigil"
    );
...
TEST(VigilSourceTest, RejectsMissingRuntime) {
    ...
    EXPECT_EQ(
        vigil_source_registry_register_cstr(&registry, "main.vigil", "", &source_id, &error),
...
TEST(VigilSourceTest, ResolvesOffsetToLineAndColumn) {
    ...
    location.offset = 7U;
    ASSERT_EQ(
        vigil_source_registry_resolve_location(&registry, &location, &error),
        VIGIL_STATUS_OK
    );
    ...
    ASSERT_EQ(
        vigil_source_registry_resolve_span_start(
            &registry,
            (vigil_source_span_t){source_id, 11U, 16U},
            &location,
            &error
        ),
        VIGIL_STATUS_OK
