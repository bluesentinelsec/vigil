...
TEST(VigilDiagnosticTest, ClearDropsItemsButKeepsListUsable) {
    ...
    ASSERT_EQ(
        vigil_diagnostic_list_append_cstr(
            &list,
            VIGIL_DIAGNOSTIC_WARNING,
            warning_span,
            "warning",
            &error
        ),
...
TEST(VigilDiagnosticTest, FormatIncludesPathLineColumnSeverityAndMessage) {
    ...
    ASSERT_EQ(
        vigil_diagnostic_format(
            &registry,
            vigil_diagnostic_list_get(&list, 0U),
            &output,
            &error
        ),
        VIGIL_STATUS_OK
    ...
    EXPECT_STREQ(
        vigil_string_c_str(&output),
        "main.vigil:2:5: error: unexpected token"
    );
