...
TEST(VigilBindingTest, FunctionTableTracksDeclarationsAndRejectsDuplicates)
{
    ...
    ASSERT_EQ(vigil_binding_function_table_append(&table, &function, &index, &error), VIGIL_STATUS_OK);
    EXPECT_EQ(index, 0U);
    EXPECT_EQ(table.count, 1U);
    EXPECT_TRUE(vigil_binding_function_table_find(&table, "main", 4U, &index, NULL));
    EXPECT_EQ(index, 0U);
    ...
    EXPECT_EQ(vigil_binding_function_table_append(&table, &duplicate, NULL, &error), VIGIL_STATUS_INVALID_ARGUMENT);
...
TEST(VigilBindingTest, ScopeStackTracksDepthShadowingAndPoppedLocals)
{
    ...
    EXPECT_TRUE(vigil_binding_type_equal(local_type, vigil_binding_type_primitive(VIGIL_TYPE_BOOL)));
