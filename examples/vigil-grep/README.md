...
A full-featured grep implementation in VIGIL demonstrating professional CLI parsing, compiled regex, and file I/O.
...
```bash
# Basic search
vigil main.vigil "pattern" file.txt

# Filenames with spaces work correctly
vigil main.vigil "hello" "/tmp/my file.txt"

# Short flags with multiple files
vigil main.vigil -i -n "PATTERN" file1.txt file2.txt file3.txt

# Recursive search
vigil main.vigil -r "pattern" directory/

# Files starting with - (use --)
vigil main.vigil "pattern" -- -weird.txt

# All features combined
vigil main.vigil -i -n -c "pattern" *.txt
```
