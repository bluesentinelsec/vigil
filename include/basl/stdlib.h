...
extern BASL_API const basl_native_module_t basl_stdlib_http;
...
extern BASL_API const basl_native_module_t basl_stdlib_parse;
...
static inline basl_status_t basl_stdlib_register_all(
    ...
    if (status != BASL_STATUS_OK) return status;
    status = basl_native_registry_add(registry, &basl_stdlib_http, error);
    if (status != BASL_STATUS_OK) return status;
    ...
    if (status != BASL_STATUS_OK) return status;
    status = basl_native_registry_add(registry, &basl_stdlib_parse, error);
    if (status != BASL_STATUS_OK) return status;
