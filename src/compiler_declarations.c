...
int vigil_program_token_is_identifier_text(
...
vigil_status_t vigil_enum_decl_grow_members(
    ...
    if (memory == NULL) {
        ...
        if (status == VIGIL_STATUS_OK) {
            memset(
                (vigil_enum_member_t *)memory + old_capacity,
                0,
...
vigil_status_t vigil_interface_method_grow_params(
    vigil_program_state_t *program,
...
vigil_status_t vigil_interface_method_grow_returns(
    vigil_program_state_t *program,
    vigil_interface_method_t *method,
    size_t minimum_capacity
) {
...
vigil_status_t vigil_program_parse_interface_method_return_types(
    vigil_program_state_t *program,
    size_t *cursor,
    ...
    if (token != NULL && token->kind == VIGIL_TOKEN_LPAREN) {
        *cursor += 1U;
        while (1) {
            ...
            status = vigil_interface_method_grow_returns(
                program,
                method,
...
vigil_status_t vigil_program_parse_global_variable_declaration(
    ...
    status = vigil_program_require_non_void_type(
        program,
        type_token == NULL ? vigil_program_eof_span(program) : type_token->span,
        declared_type,
...
vigil_status_t vigil_program_parse_constant_declaration(
    ...
    if (const_token == NULL || const_token->kind != VIGIL_TOKEN_CONST) {
        return vigil_compile_report(
            program,
            const_token == NULL ? vigil_program_eof_span(program) : const_token->span,
            "expected 'const'"
        );
    }
    vigil_program_cursor_advance(program, cursor);
...
vigil_status_t vigil_program_parse_enum_declaration(
    ...
    if (enum_token == NULL || enum_token->kind != VIGIL_TOKEN_ENUM) {
        return vigil_compile_report(
            program,
            enum_token == NULL ? vigil_program_eof_span(program) : enum_token->span,
            "expected 'enum'"
        );
    }
    vigil_program_cursor_advance(program, cursor);
    ...
    while (1) {
        ...
        if (type_token != NULL && type_token->kind != VIGIL_TOKEN_RPAREN) {
            while (1) {
                ...
                status = vigil_interface_method_grow_params(
                    program,
                    method,
        ...
        status = vigil_program_parse_interface_method_return_types(
            program,
            cursor,
...
vigil_status_t vigil_program_validate_class_interface_conformance(
    vigil_program_state_t *program,
    ...
    for (i = 0U; i < decl->implemented_interface_count; i += 1U) {
        ...
        status = vigil_class_decl_grow_interface_impls(
            program,
            decl,
        ...
        for (j = 0U; j < interface_decl->method_count; j += 1U) {
            ...
            method_decl = vigil_binding_function_table_get(
                &program->functions,
            ...
            for (param_index = 0U; param_index < interface_method->return_count; param_index += 1U) {
                if (
                    !vigil_parser_type_equal(
                        vigil_function_return_type_at(method_decl, param_index),
...
vigil_status_t vigil_program_synthesize_class_constructor(
    vigil_program_state_t *program,
    vigil_class_decl_t *decl
) {
    vigil_status_t status;
    ...
    if (
        ...
        (!vigil_parser_type_is_void(init_decl->return_type) &&
         !vigil_parser_type_is_err(init_decl->return_type))
    ) {
        return vigil_compile_report(
            program,
...
vigil_status_t vigil_program_parse_class_declaration(
    ...
    if (class_token == NULL || class_token->kind != VIGIL_TOKEN_CLASS) {
        return vigil_compile_report(
            program,
            class_token == NULL ? vigil_program_eof_span(program) : class_token->span,
            "expected 'class'"
        );
    }
    vigil_program_cursor_advance(program, cursor);
    ...
    if (vigil_program_token_is_identifier_text(program, type_token, "implements", 10U)) {
        ...
        while (1) {
            ...
            if (vigil_class_decl_implements_interface(decl, interface_index)) {
                return vigil_compile_report(
                    program,
                    vigil_program_cursor_peek(program, *cursor - 1U)->span,
                    "interface is already implemented"
                );
            }
            status = vigil_class_decl_grow_implemented_interfaces(
                program,
                decl,
    ...
    while (1) {
        ...
        if (
            type_token->kind == VIGIL_TOKEN_FN &&
            vigil_program_token_at(program, *cursor + 1U) != NULL &&
