...
static int vigil_type_kinds_match(vigil_type_kind_t left_type, vigil_type_kind_t right_type) {
    return vigil_type_kind_is_valid(left_type) &&
           vigil_type_kind_is_valid(right_type) &&
...
int vigil_type_supports_unary_operator(
...
int vigil_type_supports_binary_operator(
    ...
    switch (operator_kind) {
        case VIGIL_BINARY_OPERATOR_ADD:
            return (vigil_type_kind_is_integer(left_type) &&
                    vigil_type_kinds_match(left_type, right_type)) ||
...
int vigil_function_signature_accepts_arguments(
