...
vigil_status_t vigil_platform_is_directory(const char *path, int *out_is_dir) {
    ...
    attr = GetFileAttributesA(path);
    if (attr == INVALID_FILE_ATTRIBUTES) { *out_is_dir = 0; return VIGIL_STATUS_OK; }
...
vigil_status_t vigil_platform_self_exe(
    ...
    if (len == 0 || len >= (DWORD)buf_size) {
        if (error) { error->type = VIGIL_STATUS_INTERNAL; error->value = "GetModuleFileName failed"; error->length = 24; }
        return VIGIL_STATUS_INTERNAL;
    }
    return VIGIL_STATUS_OK;
}
...
VIGIL_API vigil_status_t vigil_platform_temp_dir(
    ...
    if (len == 0) {
        if (error) { error->type = VIGIL_STATUS_INTERNAL; error->value = "GetTempPath failed"; error->length = 18; }
        return VIGIL_STATUS_INTERNAL;
    }
...
VIGIL_API vigil_status_t vigil_platform_hostname(
    ...
    if (!GetComputerNameA(buf, &size)) {
        if (error) { error->type = VIGIL_STATUS_INTERNAL; error->value = "GetComputerName failed"; error->length = 22; }
        return VIGIL_STATUS_INTERNAL;
    }
...
VIGIL_API vigil_status_t vigil_platform_exec(
    ...
    if (!CreatePipe(&stdout_rd, &stdout_wr, &sa, 0) ||
        ...
        if (error) { error->type = VIGIL_STATUS_INTERNAL; error->value = "CreatePipe failed"; error->length = 17; }
        return VIGIL_STATUS_INTERNAL;
    }
    ...
    if (!CreateProcessA(NULL, cmdline, NULL, NULL, TRUE, 0, NULL, NULL, &si, &pi)) {
        ...
        if (error) { error->type = VIGIL_STATUS_INTERNAL; error->value = "CreateProcess failed"; error->length = 20; }
        return VIGIL_STATUS_INTERNAL;
    }
...
VIGIL_API vigil_status_t vigil_platform_dlopen(
    ...
    h = LoadLibraryA(path);
    if (!h) {
        if (error) { error->type = VIGIL_STATUS_INTERNAL; error->value = "LoadLibrary failed"; error->length = 18; }
        return VIGIL_STATUS_INTERNAL;
    }
    *out_handle = (void *)h;
    return VIGIL_STATUS_OK;
...
VIGIL_API vigil_status_t vigil_platform_dlsym(
    ...
    if (!sym) {
        if (error) { error->type = VIGIL_STATUS_INTERNAL; error->value = "GetProcAddress failed"; error->length = 21; }
        return VIGIL_STATUS_INTERNAL;
    }
    *out_sym = (void *)sym;
    return VIGIL_STATUS_OK;
...
VIGIL_API vigil_status_t vigil_platform_tcp_connect(
    ...
    *out_sock = VIGIL_INVALID_SOCKET;
    ...
    if (fd == INVALID_SOCKET) { p_freeaddrinfo(res); return VIGIL_STATUS_INTERNAL; }
...
VIGIL_API vigil_status_t vigil_platform_tcp_set_timeout(
    ...
    (void)error;
    if (!ws2_load() || !p_setsockopt) return VIGIL_STATUS_UNSUPPORTED;
...
VIGIL_API vigil_status_t vigil_platform_http_request(
    ...
    HINTERNET session = pw_Open(L"VIGIL/1.0", WINHTTP_ACCESS_TYPE_DEFAULT_PROXY,
                                 WINHTTP_NO_PROXY_NAME, WINHTTP_NO_PROXY_BYPASS, 0);
    if (!session) return VIGIL_STATUS_INTERNAL;

    HINTERNET conn = pw_Connect(session, whost, (INTERNET_PORT)port, 0);
    if (!conn) { pw_CloseHandle(session); return VIGIL_STATUS_INTERNAL; }

    DWORD flags = (strcmp(scheme, "https") == 0) ? WINHTTP_FLAG_SECURE : 0;
    HINTERNET req = pw_OpenRequest(conn, wmethod, wpath, NULL,
                                    WINHTTP_NO_REFERER, WINHTTP_DEFAULT_ACCEPT_TYPES, flags);
    if (!req) { pw_CloseHandle(conn); pw_CloseHandle(session); return VIGIL_STATUS_INTERNAL; }
...
fail:
    pw_CloseHandle(req); pw_CloseHandle(conn); pw_CloseHandle(session);
    if (error) { error->type = VIGIL_STATUS_INTERNAL; error->value = "WinHTTP request failed"; error->length = 22; }
    return VIGIL_STATUS_INTERNAL;
}
