...
static void registry_lock(void) {
    while (!basl_atomic_cas(&g_registry_lock, 0, 1)) { /* spin */ }
}

static void registry_unlock(void) {
    basl_atomic_store(&g_registry_lock, 0);
}
...
static int valid_handle(int64_t handle) {
    return handle >= 0 && handle < basl_atomic_load(&g_atomic_count);
}
...
static basl_status_t atomic_exchange_fn(basl_vm_t *vm, size_t arg_count, basl_error_t *error) {
    size_t base = basl_vm_stack_depth(vm) - arg_count;
    int64_t handle = get_i64_arg(vm, base, 0);
    int64_t val = arg_count > 1 ? get_i64_arg(vm, base, 1) : 0;
    basl_vm_stack_pop_n(vm, arg_count);
    if (!valid_handle(handle)) return push_i64(vm, 0, error);
    return push_i64(vm, basl_atomic_exchange(&g_atomics[handle], val), error);
}

static basl_status_t atomic_fetch_or_fn(basl_vm_t *vm, size_t arg_count, basl_error_t *error) {
    size_t base = basl_vm_stack_depth(vm) - arg_count;
    int64_t handle = get_i64_arg(vm, base, 0);
    int64_t val = arg_count > 1 ? get_i64_arg(vm, base, 1) : 0;
    basl_vm_stack_pop_n(vm, arg_count);
    if (!valid_handle(handle)) return push_i64(vm, 0, error);
    return push_i64(vm, basl_atomic_fetch_or(&g_atomics[handle], val), error);
}

static basl_status_t atomic_fetch_and_fn(basl_vm_t *vm, size_t arg_count, basl_error_t *error) {
    size_t base = basl_vm_stack_depth(vm) - arg_count;
    int64_t handle = get_i64_arg(vm, base, 0);
    int64_t val = arg_count > 1 ? get_i64_arg(vm, base, 1) : 0;
    basl_vm_stack_pop_n(vm, arg_count);
    if (!valid_handle(handle)) return push_i64(vm, 0, error);
    return push_i64(vm, basl_atomic_fetch_and(&g_atomics[handle], val), error);
}

static basl_status_t atomic_fetch_xor_fn(basl_vm_t *vm, size_t arg_count, basl_error_t *error) {
    size_t base = basl_vm_stack_depth(vm) - arg_count;
    int64_t handle = get_i64_arg(vm, base, 0);
    int64_t val = arg_count > 1 ? get_i64_arg(vm, base, 1) : 0;
    basl_vm_stack_pop_n(vm, arg_count);
    if (!valid_handle(handle)) return push_i64(vm, 0, error);
    return push_i64(vm, basl_atomic_fetch_xor(&g_atomics[handle], val), error);
}
...
static basl_status_t atomic_fence_fn(basl_vm_t *vm, size_t arg_count, basl_error_t *error) {
    (void)error;
    basl_vm_stack_pop_n(vm, arg_count);
    basl_atomic_fence();
...
static const basl_native_module_function_t atomic_funcs[] = {
    ...
    {"exchange",  8U,  atomic_exchange_fn,  2U, i64_i64_param,     BASL_TYPE_I64,  1U, NULL, 0, NULL, NULL},
    {"fetch_or",  8U,  atomic_fetch_or_fn,  2U, i64_i64_param,     BASL_TYPE_I64,  1U, NULL, 0, NULL, NULL},
    {"fetch_and", 9U,  atomic_fetch_and_fn, 2U, i64_i64_param,     BASL_TYPE_I64,  1U, NULL, 0, NULL, NULL},
    {"fetch_xor", 9U,  atomic_fetch_xor_fn, 2U, i64_i64_param,     BASL_TYPE_I64,  1U, NULL, 0, NULL, NULL},
    ...
    {"fence",     5U,  atomic_fence_fn,     0U, NULL,              BASL_TYPE_VOID, 1U, NULL, 0, NULL, NULL},
};
