...
static const char *vigil_test_capture_msg(
    vigil_vm_t *vm, size_t base, size_t arg_count, size_t msg_arg_index,
    ...
    if (arg_count > msg_arg_index) {
        ...
        if (vigil_nanbox_is_object(msg_val)) {
            const vigil_object_t *obj =
                (const vigil_object_t *)vigil_nanbox_decode_ptr(msg_val);
...
static vigil_status_t vigil_test_assert(
...
static vigil_status_t vigil_test_fail(
    vigil_vm_t *vm, size_t arg_count, vigil_error_t *error
) {
    size_t base = vigil_vm_stack_depth(vm) - arg_count;
...
static const vigil_native_class_method_t vigil_test_t_methods[] = {
    { "assert", 6U, vigil_test_assert, 2U, vigil_test_assert_params,
      VIGIL_TYPE_VOID, 0U, NULL, 0, NULL, 0U, 0 },
    { "fail", 4U, vigil_test_fail, 1U, vigil_test_fail_params,
      VIGIL_TYPE_VOID, 0U, NULL, 0, NULL, 0U, 0 },
};
