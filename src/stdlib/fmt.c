...
static void vigil_fmt_print_value(FILE *stream, vigil_value_t v) {
    if (vigil_nanbox_is_int(v)) {
        ...
        const vigil_object_t *obj =
            (const vigil_object_t *)vigil_nanbox_decode_ptr(v);
...
static vigil_status_t vigil_fmt_println(
...
static vigil_status_t vigil_fmt_print(
    vigil_vm_t *vm, size_t arg_count, vigil_error_t *error
) {
    size_t base;
    size_t i;
...
static vigil_status_t vigil_fmt_eprintln(
    vigil_vm_t *vm, size_t arg_count, vigil_error_t *error
) {
    size_t base;
    size_t i;
...
static const vigil_native_module_function_t vigil_fmt_functions[] = {
    {
        "println", 7U,
        vigil_fmt_println,
        1U, vigil_fmt_println_params,
        VIGIL_TYPE_VOID, 0U, NULL, 0,
        NULL, NULL
    },
    {
        "print", 5U,
        vigil_fmt_print,
        1U, vigil_fmt_print_params,
        VIGIL_TYPE_VOID, 0U, NULL, 0,
        NULL, NULL
    },
    {
        "eprintln", 8U,
        vigil_fmt_eprintln,
        1U, vigil_fmt_eprintln_params,
        VIGIL_TYPE_VOID, 0U, NULL, 0,
        NULL, NULL
    },
};

#define VIGIL_FMT_FUNCTION_COUNT \
    (sizeof(vigil_fmt_functions) / sizeof(vigil_fmt_functions[0]))

VIGIL_API const vigil_native_module_t vigil_stdlib_fmt = {
    "fmt", 3U,
    vigil_fmt_functions,
    VIGIL_FMT_FUNCTION_COUNT,
    NULL, 0U
};
