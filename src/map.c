...
static uint64_t vigil_map_hash_value(const vigil_value_t *key) {
    ...
    switch (vigil_value_kind(key)) {
        ...
        case VIGIL_VALUE_OBJECT:
            object = vigil_value_as_object(key);
            return vigil_map_hash_bytes(
                vigil_string_object_c_str(object),
                vigil_string_object_length(object)
            );
        default:
            return 0U;
    }
...
static int vigil_map_key_values_equal(
...
static int vigil_map_key_matches_string(
    ...
    return stored_text != NULL &&
           vigil_string_object_length(object) == length &&
           memcmp(stored_text, text, length) == 0;
}

static int vigil_map_validate_mutable(
    const vigil_map_t *map,
    vigil_error_t *error
) {
...
static size_t vigil_map_find_index_value(
...
static size_t vigil_map_find_index_string(
    const vigil_map_entry_t *entries,
    size_t capacity,
    const char *key,
    size_t key_length,
...
static vigil_status_t vigil_map_rebuild(
    vigil_map_t *map,
    size_t capacity,
    vigil_error_t *error
) {
    ...
    for (index = 0U; index < old_capacity; index += 1U) {
        ...
        new_index = vigil_map_find_index_value(
            new_entries,
            capacity,
            &old_entry->key,
            old_entry->hash,
...
static vigil_status_t vigil_map_ensure_capacity(
    vigil_map_t *map,
    vigil_error_t *error
) {
    size_t capacity;
...
vigil_status_t vigil_map_set(
    ...
    index = vigil_map_find_index_string(
        vigil_map_entries(map),
        map->capacity,
        key,
        key_length,
        hash,
...
vigil_status_t vigil_map_set_value(
    vigil_map_t *map,
    const vigil_value_t *key,
    const vigil_value_t *value,
    vigil_error_t *error
    ...
    index = vigil_map_find_index_value(
        vigil_map_entries(map),
        map->capacity,
        key,
        hash,
...
vigil_status_t vigil_map_set_cstr(
    vigil_map_t *map,
    const char *key,
    const vigil_value_t *value,
    vigil_error_t *error
...
const vigil_value_t *vigil_map_get(
    const vigil_map_t *map,
    const char *key,
    size_t key_length
) {
...
const vigil_value_t *vigil_map_get_cstr(
    const vigil_map_t *map,
    const char *key
) {
    if (key == NULL) {
...
const vigil_value_t *vigil_map_get_value(
    const vigil_map_t *map,
    const vigil_value_t *key
) {
    int found;
    ...
    if (
        map == NULL ||
        key == NULL ||
        !vigil_map_key_value_is_supported(key) ||
        map->capacity == 0U ||
        map->entries == NULL
    ) {
        return NULL;
...
int vigil_map_entry_at(
    ...
    for (entry_index = 0U; entry_index < map->capacity; entry_index += 1U) {
        ...
        if (seen == index) {
            if (
                vigil_value_kind(&entries[entry_index].key) != VIGIL_VALUE_OBJECT ||
...
int vigil_map_entry_value_at(
    const vigil_map_t *map,
    size_t index,
    const vigil_value_t **out_key,
    const vigil_value_t **out_value
) {
    const vigil_map_entry_t *entries;
    size_t entry_index;
...
vigil_status_t vigil_map_remove_value(
    vigil_map_t *map,
    const vigil_value_t *key,
    int *out_removed,
    vigil_error_t *error
...
vigil_status_t vigil_map_remove(
    vigil_map_t *map,
    const char *key,
    size_t key_length,
    int *out_removed,
...
vigil_status_t vigil_map_remove_cstr(
    vigil_map_t *map,
    const char *key,
    int *out_removed,
    vigil_error_t *error
