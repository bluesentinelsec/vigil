...
vigil_status_t vigil_pkg_parse_spec(
    const char *input,
...
vigil_status_t vigil_pkg_lock_add(
    ...
    if (lock->count >= lock->capacity) {
        ...
        vigil_pkg_lock_entry_t *new_entries = realloc(
            lock->entries, new_cap * sizeof(vigil_pkg_lock_entry_t));
        if (new_entries == NULL) {
            set_error(error, VIGIL_STATUS_OUT_OF_MEMORY, "out of memory");
            return VIGIL_STATUS_OUT_OF_MEMORY;
        }
...
const vigil_pkg_lock_entry_t *vigil_pkg_lock_find(
...
vigil_status_t vigil_pkg_lock_read(
    const char *path,
    ...
    for (i = 0; i < count; i++) {
        ...
        if (pkg == NULL || vigil_toml_type(pkg) != VIGIL_TOML_TABLE) continue;
        ...
        if (name_val == NULL || vigil_toml_type(name_val) != VIGIL_TOML_STRING) continue;
        if (commit_val == NULL || vigil_toml_type(commit_val) != VIGIL_TOML_STRING) continue;
        ...
        version = (version_val && vigil_toml_type(version_val) == VIGIL_TOML_STRING)
            ? vigil_toml_string_value(version_val) : NULL;
...
vigil_status_t vigil_pkg_lock_write(
    const char *path,
    const vigil_pkg_lock_t *lock,
    vigil_error_t *error
) {
...
vigil_status_t vigil_pkg_git_available(vigil_error_t *error) {
    ...
    if (status != VIGIL_STATUS_OK || exit_code != 0) {
        set_error(error, VIGIL_STATUS_INVALID_ARGUMENT,
...
vigil_status_t vigil_pkg_git_clone(
    const char *url,
    const char *dest,
    vigil_error_t *error
) {
...
vigil_status_t vigil_pkg_git_fetch(
...
vigil_status_t vigil_pkg_git_checkout(
    const char *repo_path,
    const char *version,
    vigil_error_t *error
) {
...
vigil_status_t vigil_pkg_git_head(
    const char *repo_path,
    char *out_commit,
    size_t commit_size,
    vigil_error_t *error
...
static vigil_status_t read_project_toml(
    const char *project_root,
    vigil_toml_value_t **out_root,
    vigil_error_t *error
) {
    char toml_path[4096];
    char *data = NULL;
    ...
    if (vigil_platform_path_join(project_root, "vigil.toml", toml_path,
...
static vigil_status_t write_project_toml(
    const char *project_root,
...
static vigil_status_t ensure_deps_table(
    vigil_toml_value_t *root,
    vigil_toml_value_t **out_deps,
    vigil_error_t *error
) {
...
static vigil_status_t install_transitive_deps(
    const char *project_root,
    const char *pkg_path,
    vigil_pkg_lock_t *lock,
    vigil_error_t *error
...
static vigil_status_t install_package(
    const char *project_root,
    const char *pkg_url,
    const char *version,
    vigil_pkg_lock_t *lock,
    ...
    /* Build paths */
    if (vigil_platform_path_join(project_root, "deps", deps_path,
            sizeof(deps_path), error) != VIGIL_STATUS_OK) {
        return error->type;
    }
    if (vigil_platform_path_join(deps_path, pkg_url, pkg_path,
            sizeof(pkg_path), error) != VIGIL_STATUS_OK) {
        return error->type;
    }
...
static vigil_status_t install_transitive_deps(
    ...
    if (vigil_platform_path_join(pkg_path, "vigil.toml", toml_path,
            sizeof(toml_path), error) != VIGIL_STATUS_OK) {
        return VIGIL_STATUS_OK; /* Not fatal */
    }

    vigil_platform_file_exists(toml_path, &exists);
    if (!exists) return VIGIL_STATUS_OK;
...
vigil_status_t vigil_pkg_get(
    ...
    /* Build lock path */
    if (vigil_platform_path_join(project_root, "vigil.lock", lock_path,
            sizeof(lock_path), error) != VIGIL_STATUS_OK) {
        goto cleanup;
    }
...
static vigil_status_t remove_dir_callback(const char *name, int is_dir, void *user_data) {
    ...
    if (strcmp(name, ".") == 0 || strcmp(name, "..") == 0) return VIGIL_STATUS_OK;
...
vigil_status_t vigil_pkg_remove(
    const char *project_root,
    ...
    if (vigil_platform_path_join(deps_path, package_url, pkg_path,
            sizeof(pkg_path), error) != VIGIL_STATUS_OK) {
        return error->type;
    }
    if (vigil_platform_path_join(project_root, "vigil.lock", lock_path,
...
vigil_status_t vigil_pkg_resolve_import(
    const char *project_root,
    ...
    if (vigil_platform_path_join(deps_path, import_path, candidate,
            sizeof(candidate), error) != VIGIL_STATUS_OK) {
        return error->type;
    }
    {
    ...
    /* Try deps/<package>/lib/<module>.vigil */
    {
        ...
        if (pkg_end != NULL && module_start != NULL && *module_start != '\0') {
            ...
            if (vigil_platform_path_join(deps_path, pkg_root, candidate,
                    sizeof(candidate), error) == VIGIL_STATUS_OK &&
                vigil_platform_path_join(candidate, "lib", lib_path,
                    sizeof(lib_path), error) == VIGIL_STATUS_OK &&
                vigil_platform_path_join(lib_path, module_file, candidate,
                    sizeof(candidate), error) == VIGIL_STATUS_OK) {
    ...
    if (vigil_platform_path_join(deps_path, import_path, candidate,
        ...
        char entry[4096];
        char mod_path[4096];
        if (vigil_platform_path_join(candidate, "lib", entry,
                sizeof(entry), error) == VIGIL_STATUS_OK &&
            vigil_platform_path_join(entry, "mod.vigil", mod_path,
                sizeof(mod_path), error) == VIGIL_STATUS_OK) {
