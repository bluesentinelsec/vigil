...
basl_status_t basl_pkg_lock_write(
    ...
    buf_len = (size_t)snprintf(buf, buf_size, "# AUTO-GENERATED by basl get - DO NOT EDIT\n\n");
    ...
    status = basl_platform_write_file(path, buf, buf_len, error);
    free(buf);
    return status;
}
...
basl_status_t basl_pkg_git_clone(
    ...
    if (exit_code != 0) {
        free(out);
        free(err_out);
        set_error(error, BASL_STATUS_INTERNAL, "git clone failed");
...
basl_status_t basl_pkg_git_checkout(
    ...
    if (exit_code != 0) {
        free(out);
        free(err_out);
        set_error(error, BASL_STATUS_INVALID_ARGUMENT, "version not found");
        return BASL_STATUS_INVALID_ARGUMENT;
    }
...
basl_status_t basl_pkg_resolve_import(
    ...
    /* Try deps/<package>/lib/<module>.basl */
    {
        ...
        if (pkg_end != NULL && module_start != NULL && *module_start != '\0') {
            ...
            snprintf(module_file, sizeof(module_file), "%s.basl", module_start);
            ...
            if (basl_platform_path_join(deps_path, pkg_root, candidate,
                ...
                basl_platform_path_join(lib_path, module_file, candidate,
                    sizeof(candidate), error) == BASL_STATUS_OK) {
    ...
    if (basl_platform_path_join(deps_path, import_path, candidate,
        ...
        if (basl_platform_path_join(candidate, "lib", entry,
            ...
            basl_platform_path_join(entry, "mod.basl", mod_path,
                sizeof(mod_path), error) == BASL_STATUS_OK) {
