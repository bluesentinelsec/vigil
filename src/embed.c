...
static vigil_status_t embed_read_files(const char **file_paths, size_t file_count, char ***out_datas,
                                       size_t **out_sizes, vigil_error_t *error)
{
    ...
    *out_datas = file_datas;
    *out_sizes = sizes;
    return VIGIL_STATUS_OK;
}

static void embed_emit_byte_arrays(buf_t *out, char **file_datas, const size_t *sizes, size_t file_count)
{
    for (size_t i = 0; i < file_count; i++)
    {
                                 ...
                                 size_t *out_length, vigil_error_t *error)
{
    ...
    status = embed_read_files(file_paths, file_count, &file_datas, &sizes, error);
    if (status != VIGIL_STATUS_OK)
        return status;
    ...
    embed_emit_byte_arrays(&out, file_datas, sizes, file_count);
