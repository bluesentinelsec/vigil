...
func TestServeConditionalBreakpointsStopWhenConditionTurnsTrue(t *testing.T) {
	root := t.TempDir()
	mainPath := filepath.Join(root, "main.basl")
	if err := os.WriteFile(mainPath, []byte(`import "fmt";
...
`), 0644); err != nil {
	...
	for {
		...
		case msg.Type == "event" && msg.Event == "stopped":
			var stopped struct {
				Reason string `json:"reason"`
			}
			decodeBody(t, msg.Body, &stopped)
			if !sawFirstOutput {
				t.Fatal("conditional breakpoint stopped before first iteration output")
			}
			if stopped.Reason != "breakpoint" {
				t.Fatalf("stopped reason = %#v, want breakpoint", stopped)
			}
			goto continued
		default:
			t.Fatalf("unexpected message before conditional stop = %#v", msg)
		}
	}
...
continued:
	...
	var sawExited bool
	var sawTerminated bool
	for !(sawContinue && sawExited && sawTerminated) {
		msg = readProtocolMessage(t, reader)
		switch {
		case msg.Type == "response" && msg.Command == "continue":
			sawContinue = true
		...
		case msg.Type == "event" && msg.Event == "terminated":
			sawTerminated = true
		default:
			t.Fatalf("unexpected message after conditional continue = %#v", msg)
		}
	}
...
func TestServeLogpointsRespectHitConditionsWithoutStopping(t *testing.T) {
	root := t.TempDir()
	mainPath := filepath.Join(root, "main.basl")
	if err := os.WriteFile(mainPath, []byte(`fn main() -> void {
    for (i32 i = 0; i < 3; i++) {
        i32 value = i;
    }
}
`), 0644); err != nil {
	...
	sendRequest(t, clientConn, 3, "setBreakpoints", map[string]any{
		...
		"breakpoints": []map[string]any{
			{"line": 3, "hitCondition": "2", "logMessage": "i={i}"},
		},
	})
	msg = readProtocolMessage(t, reader)
	if msg.Type != "response" || !msg.Success || msg.Command != "setBreakpoints" {
	...
	var sawExited bool
	var sawTerminated bool
	var sawLogpoint bool
	for !(sawExited && sawTerminated) {
		msg = readProtocolMessage(t, reader)
		switch {
		case msg.Type == "event" && msg.Event == "output":
			var output struct {
		...
		case msg.Type == "event" && msg.Event == "stopped":
			t.Fatalf("logpoint unexpectedly stopped execution: %#v", msg)
		case msg.Type == "event" && msg.Event == "exited":
			sawExited = true
		case msg.Type == "event" && msg.Event == "terminated":
			sawTerminated = true
		default:
			t.Fatalf("unexpected message during logpoint run = %#v", msg)
		}
	}
	if !sawLogpoint {
		t.Fatal("expected logpoint output for second hit")
	}
